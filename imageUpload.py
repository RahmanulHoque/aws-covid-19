...
bucket_name = 'facemaskdetectionbucket'
	...
	try:
		...
		while doInfer:
			...
			for obj in parsed_results:
				...
				if (time_now >= cooldown) and obj['prob'] >= 0.10:
					...
					if onCountdown and time_now >= countdown:
						...
						result = s3.put_object(ACL='public-read', Body=jpg_data.tostring(), Bucket=bucket_name, Key=key)

						message = "uploaded to s3: " + key
						client.publish(topic=iotTopic, payload = message)
						cooldown = time_now + datetime.timedelta(seconds = 0)
						onCountdown = False
					# Starting countdown
					elif not onCountdown:
						onCountdown = True
						countdown = time_now + datetime.timedelta(seconds = 0)
